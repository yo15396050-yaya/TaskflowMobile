...
export default function SocialSituationScreen() {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await api.get('/situations-social/user');
      setData(response.data);
    } catch (error) {
      console.error('Erreur chargement situation sociale:', error);
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
    ...
    <View style={[styles.card, { backgroundColor: themeColors.cardBackground, borderColor: themeColors.border }]}>
      <View style={styles.cardHeader}>
        <View style={styles.typeBadge}>
          ...
          <Text style={[styles.typeName, { color: '#3498db' }]}>{item.type}</Text>
        </View>
      ...
      <View style={styles.cardFooter}>
        ...
        {item.document && (
          <TouchableOpacity 
            style={[styles.downloadBtn, { backgroundColor: '#3498db' }]}
            onPress={() => Linking.openURL(`https://flowtask.dc-knowing.com/user-uploads/situation-sociale/${item.document}`)}
          >
            <Ionicons name="download-outline" size={18} color="#FFF" />
            <Text style={[styles.downloadText, { color: '#FFF' }]}>Attestation</Text>
          </TouchableOpacity>
        )}
      </View>
    </View>
    ...
    <View style={[styles.container, { backgroundColor: themeColors.background }]}>
      <View style={[styles.header, { backgroundColor: '#181818' }]}>
        <View style={styles.headerTop}>
          ...
          <Text style={styles.headerTitle}>Situation Sociale</Text>
          <View style={{ width: 40 }} />
        </View>
        <Text style={styles.headerSubtitle}>Suivi des cotisations sociales (CNPS...)</Text>
      </View>
        ...
        ListEmptyComponent={
          loading ? (
            ...
            <View style={styles.emptyContainer}>
              <Ionicons name="people-circle-outline" size={60} color="#888" />
              <Text style={[styles.emptyText, { color: themeColors.textSecondary }]}>Aucune donnée sociale trouvée</Text>
            </View>
          )
        }
      />
