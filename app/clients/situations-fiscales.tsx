...
export default function SituationsFiscalesScreen() {
  ...
  const fetchFiscalData = async () => {
    try {
      ...
      const userId = user?.id || '531'; // Fallback sur votre ID si besoin
      
      const response = await api.get(`/dashboard/situationsFiscal/${userId}`);
      setFiscalData(response.data.data || response.data || []);
    } catch (error) {
      console.error('Erreur fiscal:', error);
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
    ...
    return (
      ...
      >
        ...
        <View style={styles.cardTop}>
          <View style={{ flex: 1 }}>
            <Text style={[styles.clientName, { color: themeColors.text }]} numberOfLines={1}>{item.client || item.name || 'Client inconnu'}</Text>
            <Text style={[styles.ncc, { color: themeColors.textSecondary }]}>NCC: {item.ncc || 'N/A'}</Text>
          </View>
        ...
        <View style={styles.infoGrid}>
          <View style={styles.infoItem}>
            <Text style={[styles.infoLabel, { color: themeColors.textSecondary }]}>Régime</Text>
            <Text style={[styles.infoValue, { color: themeColors.text }]}>{item.regime || 'N/A'}</Text>
          </View>
          <View style={styles.infoItem}>
            <Text style={[styles.infoLabel, { color: themeColors.textSecondary }]}>Activité</Text>
            <Text style={[styles.infoValue, { color: themeColors.text }]}>{item.activite || 'N/A'}</Text>
          </View>
        </View>

        {/* Bottom: Last Declaration + Amount */}
        <View style={[styles.cardBottom, { borderTopColor: themeColors.border }]}>
          <View style={styles.bottomItem}>
            ...
            <Text style={[styles.bottomText, { color: themeColors.textSecondary }]}>Dern. décl: {item.lastDecl || item.date || 'N/A'}</Text>
          </View>
    ...
    <View style={[styles.container, { backgroundColor: themeColors.background }]}>
      ...
      <View style={styles.header}>
        <View style={styles.headerTop}>
          ...
          <Text style={styles.headerTitle}>Situations Fiscales</Text>
          <View style={{ width: 24 }} />
        </View>
        ...
        <View style={styles.summaryRow}>
          <View style={styles.summaryItem}>
            <Text style={styles.summaryNumber}>{fiscalData.filter(d => d.status === 'À jour').length}</Text>
            <Text style={styles.summaryLabel}>À jour</Text>
          </View>
          <View style={[styles.summaryItem, styles.summaryDivider]}>
            <Text style={[styles.summaryNumber, { color: '#e74c3c' }]}>{fiscalData.filter(d => d.status === 'En retard').length}</Text>
            <Text style={styles.summaryLabel}>En retard</Text>
          </View>
          <View style={styles.summaryItem}>
            <Text style={[styles.summaryNumber, { color: '#f1c40f' }]}>{fiscalData.filter(d => d.status === 'En attente').length}</Text>
...
const styles = StyleSheet.create({
  ...
  ncc: { fontSize: 12, fontWeight: '600' },
  ...
  infoItem: { flex: 1 },
  infoLabel: { fontSize: 10, fontWeight: '700', textTransform: 'uppercase', marginBottom: 3 },
